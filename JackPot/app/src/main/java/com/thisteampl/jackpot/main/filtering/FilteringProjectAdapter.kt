...
class FilteringProjectAdapter(
    val filteringprojectadapt: List<ProjectElementMaterial> = mutableListOf()): RecyclerView.Adapter<FilteringProjectAdapter.filteringprojectViewHolder>() {

    class filteringprojectViewHolder(view: View) : RecyclerView.ViewHolder(view)

    // onCreateViewHolder : ViewHolder와 Layout 파일을 연결해주는 역할
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): filteringprojectViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.holder_filtered_project_list, parent, false)

        return filteringprojectViewHolder(view)
    }

    override fun getItemCount(): Int {
        if (filteringprojectadapt != null) {
            return filteringprojectadapt.size
        } else {
            return 0
        }
    }
    ...
    override fun onBindViewHolder(holder: filteringprojectViewHolder, position: Int) {

        val item = filteringprojectadapt[position]
